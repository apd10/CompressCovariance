...
def miss_prop_at_t(Tau, n, R, k, sigma, signal, alpha, T, t):
    order_expect = -norm.ppf(((k - 1) / 2 - 0.375) / (k + 1 - 2 * 0.375))
    sigma_x = sigma * np.sqrt(1 / t + math.pi * (n - 1) * (1 - alpha) / (2 * k * t * (R - alpha)))
    # target_signal-Tau*T/exploration_period
    x = signal - Tau*T/t
    x2 = signal + Tau*T/t
    percentile_1 = -x / sigma_x
    percentile_11 = -x2 / sigma_x
    percentile_2 = -(x / sigma_x - order_expect)
    percentile_21 = -(x2 / sigma_x - order_expect)
    p = 0.5 + 0.5 * ((R - alpha) / R) ** (n - 1)
    prob_1 = p ** k
    prob_2 = k * (1 - p) * p ** (k - 1)
    
    # print('Saturating probability:', 1 - prob_1 - prob_2)
    # print('Negative probability:', prob_1 * norm.cdf(percentile_11))
    return prob_1 * (norm.cdf(percentile_1)-norm.cdf(percentile_11)) + \
            prob_2 * (norm.cdf(percentile_2)-norm.cdf(percentile_21)) + (1 - prob_1 - prob_2)
...
def miss_prop_after_t(theta, Tau, n, R, k, sigma, signal, alpha, T, t):
    ...
    return a1 * (prob_1+norm.cdf(percentile_11)) + a2 * (prob_2+norm.cdf(percentile_21)) 
...
def find_best_exploration_period(signal, alpha, init_threshold, num_features, cs_params, total_samples, target_miss_probability):
    ...
    while(end > start + 1):
      mid = int((start + end)/2)
      prob = miss_prop_at_t(init_threshold, n=num_features, R=cs_params["R"],
...
if __name__ == '__main__':
    ...
    exp = find_best_exploration_period(signal, alpha, Tau, n, {"K":k, "R":R }, T, 0.05)
    print("Exploration Period:", exp)
    theta  = find_best_theta(signal, alpha, Tau, n, {"K":k, "R":R }, T, exp, 0.15)
    print("theta:", theta)
